...
async function barcode(){
  try{
    if(window.CapacitorBarcodeScanner?.scanBarcode){let r=await window.CapacitorBarcodeScanner.scanBarcode({scanInstructions:"Enfoca el código",android:{scanningLibrary:"zxing"}});let code=r.ScanResult||r.content;findBarcode(code)}
    else if("BarcodeDetector" in window){let d=new BarcodeDetector();toast("Escáner web disponible; usa la cámara desde el navegador.")}
    else {let code=prompt("Código de barras:", "");if(code)findBarcode(code)}
  }catch(e){let code=prompt("No se pudo abrir el lector. Escribe el código:","");if(code)findBarcode(code)}
}
...
function customerPick(){modal('<h2>Cliente</h2><div class="list">'+('<button class="row menu-row" onclick="state.customerId=null;close();render()"><b>Consumidor final</b></button>')+db.customers.map(c=>'<button class="row menu-row" onclick="state.customerId='+c.id+';close();render()"><div class="grow"><b>'+esc(c.name)+'</b><small>'+esc(c.phone||"")+'</small></div></button>').join("")+'</div><button class="btn" onclick="customerEdit();">+ Nuevo cliente</button>')}
function checkout(){
